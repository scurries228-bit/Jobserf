...
Готов продолжить свой путь? 🚀""")
    ...
    # Кнопка должна находиться прямо под сообщением.
    # Используем InlineKeyboard + callback_data=reg_start.
    k=types.InlineKeyboardMarkup()
    k.add(types.InlineKeyboardButton("🚀 Начать!", callback_data="reg_start"))
...
def reg_start_message(message):
    uid=message.from_user.id
    if uid in players:
        show_main_menu(message.chat.id,uid)
        return
    registration[uid]=True
    text="""[ 👤 ] <b>Регистрация в Jobserf.</b>
...
def reg_start(call):
    ...
    # Сразу подтверждаем callback, чтобы Telegram не показывал вечную загрузку.
    try:
...
def p_offer(call):
    ...
    try:
        ...
        bot.answer_callback_query(call.id,"Некорректный игрок.",show_alert=True);return
    tb=business(target);b=business(uid)
    if not b or not tb or target==uid:
        bot.answer_callback_query(call.id,"Компания недоступна.",show_alert=True);return
    if any(x.get("from")==uid and x.get("status")=="pending" for x in tb.get("offers_in",[])):
        bot.answer_callback_query(call.id,"Предложение уже отправлено.");return
    offer={"from":uid,"to":target,"status":"pending","created_at":int(time.time())}
    tb['offers_in'].append(offer);b['offers_out'].append(offer)
    save_data()
    k=types.InlineKeyboardMarkup(row_width=2)
    k.row(types.InlineKeyboardButton("✅ Принять",callback_data=f"partner_accept_{uid}"),types.InlineKeyboardButton("❌ Отказать",callback_data=f"partner_decline_{uid}"))
    bot.send_message(target,f"[ 🤝 ] <b>Новое предложение о сотрудничестве.</b>\n\n<blockquote>🏢 Компания: {esc(b['name'])}\n\n👤 Владелец: {esc(players[uid]['nickname'])}</blockquote>\n\nХочешь принять предложение?",reply_markup=k)
    bot.answer_callback_query(call.id,"Предложение отправлено.")

@bot.callback_query_handler(func=lambda c:c.data.startswith("partner_accept_"))
def partner_accept(call):
    target=call.from_user.id
    try: sender=int(call.data.split("_")[-1])
    except ValueError:
        bot.answer_callback_query(call.id,"Некорректное предложение.",show_alert=True);return
    b=business(sender);tb=business(target)
    if not b or not tb:
        bot.answer_callback_query(call.id,"Предложение больше недоступно.",show_alert=True);return
    offer=next((x for x in tb.get("offers_in",[]) if x.get("from")==sender and x.get("status")=="pending"),None)
    if not offer:
        bot.answer_callback_query(call.id,"Предложение уже обработано.",show_alert=True);return
    offer["status"]="accepted"
    partner_a={"owner_id":target,"name":tb["name"]}
    partner_b={"owner_id":sender,"name":b["name"]}
    b.setdefault("partners",[]).append(partner_a)
    tb.setdefault("partners",[]).append(partner_b)
    contract={"from":sender,"to":target,"name":tb["name"],"started_at":int(time.time())}
    b.setdefault("contracts",[]).append(contract)
    tb.setdefault("contracts",[]).append({"from":sender,"to":target,"name":b["name"],"started_at":int(time.time())})
    record_op(b,"partner",0,f"Заключён договор с {tb['name']}")
    record_op(tb,"partner",0,f"Заключён договор с {b['name']}")
    save_data()
    bot.send_message(sender,f"[ 🤝 ] <b>Сотрудничество подтверждено.</b>\n\nИгрок <b>{esc(players[target]['nickname'])}</b> принял предложение по бизнесу <b>{esc(tb['name'])}</b>.")
    edit_callback(call,"[ ✅ ] <b>Сотрудничество подтверждено.</b>\n\nДоговор успешно создан.",partners_menu())
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c:c.data.startswith("partner_decline_"))
def partner_decline(call):
    target=call.from_user.id
    try: sender=int(call.data.split("_")[-1])
    except ValueError:
        bot.answer_callback_query(call.id,"Некорректное предложение.",show_alert=True);return
    ...
    if sender in players:
        bot.send_message(sender,f"[ ❌ ] <b>Предложение отклонено.</b>\n\nИгрок <b>{esc(players[target]['nickname'])}</b> отказался от сотрудничества.")
    edit_callback(call,"[ ❌ ] <b>Предложение отклонено.</b>",partners_menu())
    bot.answer_callback_query(call.id)
...
# ---------------------------------------------------------------------------
# Render-safe webhook server
# ---------------------------------------------------------------------------
# Jobserf previously used getUpdates/infinity_polling. On Render this can
# produce Telegram 409 conflicts when an old instance is still alive.
# A webhook gives Telegram exactly one delivery endpoint and avoids the
# getUpdates race completely.

PORT = int(os.getenv("PORT", "10000"))
RENDER_EXTERNAL_URL = os.getenv("RENDER_EXTERNAL_URL", "").strip().rstrip("/")
RENDER_EXTERNAL_HOSTNAME = os.getenv("RENDER_EXTERNAL_HOSTNAME", "").strip().strip("/")
...
class TelegramWebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.split("?", 1)[0] == "/health":
            self._send(200, b"Jobserf is running")
        else:
            self._send(200, b"OK")
    ...
    def do_POST(self):
        ...
        try:
            ...
            if not raw:
                self._send(400, b"Empty request")
                return

            update = telebot.types.Update.de_json(raw.decode("utf-8"))
            if update is not None:
                # Never let one malformed Telegram update kill the web server.
                try:
                    bot.process_new_updates([update])
                except Exception as exc:
                    print("update processing error:", repr(exc), flush=True)

            self._send(200, b"OK")
        except Exception as exc:
            print("webhook error:", repr(exc), flush=True)
            # Telegram only needs a quick HTTP response; the service stays up.
            self._send(200, b"OK")
...
def run_webhook_server():
    if not WEBHOOK_URL:
        raise RuntimeError(
            "WEBHOOK_URL is not configured. Set WEBHOOK_URL to your Render public URL."
        )

    full_webhook_url = WEBHOOK_URL + WEBHOOK_PATH

    # Setting the webhook replaces any old webhook endpoint. More importantly,
    # this service never calls getUpdates, so it cannot create a 409 polling
    # conflict itself.
    print("Setting Telegram webhook:", full_webhook_url, flush=True)
    bot.remove_webhook()
    time.sleep(0.5)
    bot.set_webhook(url=full_webhook_url, drop_pending_updates=True)
    print("Telegram webhook is active.", flush=True)

    server = ThreadingHTTPServer(("0.0.0.0", PORT), TelegramWebhookHandler)
    print(f"Jobserf web server listening on 0.0.0.0:{PORT}", flush=True)
    server.serve_forever()


if __name__ == "__main__":
    print("Jobserf запущен.", flush=True)
    run_webhook_server()
