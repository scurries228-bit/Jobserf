...
Готов продолжить свой путь? 🚀""")
    ...
    # Используем обычную Telegram-кнопку вместо inline callback на первом экране.
    # Так кнопка "🚀 Начать!" работает даже если callback_query временно не приходит.
    k=types.ReplyKeyboardMarkup(resize_keyboard=True,one_time_keyboard=True)
    k.add(types.KeyboardButton("🚀 Начать!"))
