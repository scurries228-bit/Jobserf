...
Готов продолжить свой путь? 🚀""")
    ...
    k=types.InlineKeyboardMarkup()
    k.add(types.InlineKeyboardButton("🚀 Начать!", callback_data="START_REGISTRATION"))
...
def reg_start(call):
    ...
    # Обязательно закрываем состояние загрузки Telegram
    try:
...
✏️ Отправь своё игровое имя следующим сообщением. От <u>3 до 20 символов</u>."""
    ...
    # Не редактируем старое сообщение: отправляем новое.
    # Это надёжнее для Telegram/Render и исключает зависание callback.
    try:
        bot.send_message(chat_id, text, reply_markup=types.ReplyKeyboardRemove())
    except Exception as e:
        print("reg_start send error:", e)
