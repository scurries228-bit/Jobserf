...
def admin_panel(message):
    ...
    k.add(types.InlineKeyboardButton("📊 Статистика",callback_data="admin_stats"))
    k.add(types.InlineKeyboardButton("📊 Статистика",callback_data="admin_stats"))
...
def give_money(call):
    ...
    admin_states[uid]={"state":"give_money_id"}
    edit_callback(call,"[ 💰 ] <b>Выдача денег.</b>\n\nОтправь Telegram ID пользователя, которому хочешь выдать деньги.",inline_back("admin_back"))
    bot.answer_callback_query(call.id)

@bot.message_handler(content_types=["text"],func=lambda m:is_admin(m.from_user.id) and m.from_user.id in admin_states and admin_states[m.from_user.id].get('state')=='give_money_id')
def give_money_id_msg(message):
    uid=message.from_user.id
    try:
        target=int(message.text.strip())
    except ValueError:
        bot.send_message(message.chat.id,"[ ❌ ] Telegram ID должен состоять только из цифр.")
        return
    if target not in players:
        bot.send_message(message.chat.id,"[ ❌ ] Пользователь с таким Telegram ID не зарегистрирован в Jobserf.")
        return
    admin_states[uid]={"state":"give_money_amount","target":target}
    bot.send_message(message.chat.id,f"[ 💰 ] <b>Сумма зачисления.</b>\n\nПользователь: <b>{esc(players[target]['nickname'])}</b>\nTelegram ID: <code>{target}</code>\n\nОтправь сумму от 0 до 1.000.000.000₽.")

@bot.message_handler(content_types=["text"],func=lambda m:is_admin(m.from_user.id) and m.from_user.id in admin_states and admin_states[m.from_user.id].get('state')=='give_money_amount')
def give_money_amount_msg(message):
    uid=message.from_user.id
    ...
    try:
        ...
        bot.send_message(message.chat.id,"[ ❌ ] Укажи сумму только числом, например: 392929")
        return
    if not 0<=amount<=1_000_000_000:
        bot.send_message(message.chat.id,"[ ❌ ] Сумма должна быть от 0 до 1.000.000.000₽.")
        return
    target=state["target"]
    if target not in players:
        admin_states.pop(uid,None)
        bot.send_message(message.chat.id,"[ ❌ ] Пользователь больше не найден.",reply_markup=inline_back("admin_back"))
        return
    players[target]["balance"]+=amount
    admin_states.pop(uid,None)
    save_data()
    bot.send_message(message.chat.id,f"[ ✅ ] <b>Деньги выданы.</b>\n\nПользователю с Telegram ID <code>{target}</code> успешно зачислено {format_money(amount)}.",reply_markup=inline_back("admin_back"))
    if players[target].get("notifications",True):
        try:
            bot.send_message(target,f"[ 💰 ] <b>Зачисление средств.</b>\n\nАдминистратор Jobserf зачислил на твой баланс <b>{format_money(amount)}</b>.\n\n💳 Текущий баланс: <b>{format_money(players[target]['balance'])}</b>.")
        except Exception:
            pass
...
def admin_stats(call):
    if not is_admin(call.from_user.id): return
    total=len(players)
    bot.send_message(call.message.chat.id,f"[ 📊 ] <b>Статистика Jobserf.</b>\n\n━━━━━━━━━━━━━━━━━━\n\n<blockquote>👥 Зарегистрировано пользователей: <b>{total}</b></blockquote>\n\n━━━━━━━━━━━━━━━━━━",reply_markup=inline_back("admin_back"))
    bot.answer_callback_query(call.id)
